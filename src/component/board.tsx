...
const Board: React.FC<BoardProps> = props => {
  ...
  const boardView = (
    array.map((val, idx) => { return (<button key={'btn' + idx} className={btnClassName(val)} style={{ backgroundColor: btnColor(val), borderRight: borderRight(idx), borderBottom: borderBottom(idx), width: onSetCubeLength(), height: onSetCubeLength() }} onClick={() => onClickButton(idx)}></button>) })
  )
