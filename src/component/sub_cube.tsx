        ...
        <div style={{ padding: '10px' }}>
            <div className="basic_cube" style={{ width: onSetWidthLength() }}>
                {inputArray.flat().map((val, idx) => {
                  return (<button key={'sub_box_' + editIndex + '_btn_' + idx} className={btnClassName(val)} style={{ backgroundColor: btnColor(val, idx), width: onSetCubeLength(), height: onSetCubeLength() }} onClick={() => onClickButton(idx)}></button>)
                })}
